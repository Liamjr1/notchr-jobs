...
const statusIcons: Record<string, React.ReactNode> = {
  ...
  reviewing: <TrendingUp className="w-3.5 h-3.5" />,
  shortlisted: <CheckCircle className="w-3.5 h-3.5" />,
  interview: <CheckCircle className="w-3.5 h-3.5" />,
  offered: <CheckCircle className="w-3.5 h-3.5" />,
...
export default function DashboardPage() {
  ...
  const stats = [
    { label: 'Total Applied', value: applications.length, icon: Briefcase, color: 'text-blue-600', bg: 'bg-blue-50' },
    { label: 'In Progress', value: applications.filter(a => ['reviewing', 'shortlisted', 'interview'].includes(a.status)).length, icon: TrendingUp, color: 'text-purple-600', bg: 'bg-purple-50' },
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-1">Track your job applications and saved jobs</p>
          </div>
          <Link
            href="/jobs"
            className="bg-blue-600 hover:bg-blue-700 text-white font-medium px-5 py-2.5 rounded-lg transition-colors inline-flex items-center gap-2"
          >
          ...
          <div>
            {applications.length === 0 ? (
              <div className="bg-white rounded-xl border border-gray-100 p-16 text-center">
                ...
                <Link href="/jobs" className="bg-blue-600 hover:bg-blue-700 text-white font-medium px-6 py-2.5 rounded-lg transition-colors inline-flex items-center gap-2">
                    ...
                    <div key={app._id} className="bg-white rounded-xl border border-gray-100 p-6">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex items-start gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-gray-500 text-sm mb-2">
                              {(job?.company as unknown as { name: string })?.name || 'Company'}
                            </p>
                        ...
                        <div className="flex flex-col items-end gap-3">
                          <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium capitalize ${statusColors[app.status]}`}>
